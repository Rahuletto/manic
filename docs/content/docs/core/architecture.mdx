...
The build system is now modularizing into a split architecture: framework orchestration in `manicjs` + reusable bundler primitives in `@manicjs/bundler`.
...
- `manicjs` owns CLI orchestration and plugin/provider coordination.
- `@manicjs/bundler` owns low-level bundler primitives (resolver, stats helpers, minify helpers) used by `manic build`.
