...
`manic start` teardown targets only the spawned production server process and listener-owner cleanup for the selected port. Connected client apps (such as browsers) are not treated as kill targets during normal shutdown.
